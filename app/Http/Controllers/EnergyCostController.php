...
class EnergyCostController extends Controller
{
    ...
    {	
        ...
        foreach($energySystems as $energySystem) {
            ...
            $relayCost = EnergySystemRelayDriver::where('energy_system_id', $energySystem->id)
                ->value('cost');
            $loggerCost = EnergySystemMonitoring::where('energy_system_id', $energySystem->id)
                ->value('cost');
            ...
            $totalCost += $batteryCost ?? 0;
            $totalCost += $batteryMountCost ?? 0;
            $totalCost += $pvCost ?? 0;
            $totalCost += $pvMountCost ?? 0;
            $totalCost += $relayCost ?? 0;
            $totalCost += $loggerCost ?? 0;
            $totalCost += $ccCost ?? 0;
            $totalCost += $turbineCost ?? 0;
            $totalCost += $generatorCost ?? 0;
            $totalCost += $bspCost ?? 0;
            $totalCost += $btsCost ?? 0;
            $totalCost += $inventerCost ?? 0;
            $totalCost += $loadCost ?? 0;
            $totalCost += $pvMcbCost ?? 0;
            $totalCost += $mcbCcCost ?? 0;
            $totalCost += $rccCost ?? 0;
            $totalCost += $mcbInventerCost ?? 0;
            $totalCost += $airCost ?? 0;
            $totalCost += $electricityRoomCost ?? 0;
            $totalCost += $electricityRoomBosCost ?? 0;
            $totalCost += $gridCost ?? 0;
            $totalCost += $houseWiringCost ?? 0;
    ...
    public function edit($id)
    {
        ...
        return view('costs.energy.edit', compact('energySystem', 'battarySystems', 'pvSystems', 
            ...
            'gridRoom', 'battaryMountSystems', 'pvMountSystems', 'houseWiringSystem'));
    }
