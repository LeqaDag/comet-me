...
class DonorEnergy implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, WithStyles
{
        ...
        $data =  DB::table('all_energy_meters')
            ...
            ->select(
                ...
                DB::raw('COUNT(DISTINCT CASE WHEN households.id IS NOT NULL THEN 
                    all_energy_meters.id END) as served_households'),
                ...
                DB::raw('COUNT(DISTINCT CASE WHEN public_structures.id IS NOT NULL 
                    THEN all_energy_meters.id END) as served_public_structures'),
