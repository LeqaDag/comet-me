...
class EnergyInverter implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    */
    public function collection()  
    {
        $query = DB::table('energy_systems')
            ...
            ->LeftJoin('energy_system_inverters', 'energy_systems.id', '=', 
                'energy_system_inverters.energy_system_id')
            ->LeftJoin('energy_inverters', 'energy_system_inverters.energy_inverter_id', '=', 
                'energy_inverters.id')
            ->where('energy_systems.is_archived', 0)
            ->select(
                'energy_systems.name', 
                'energy_inverters.inverter_model', 'energy_system_inverters.inverter_units',
            );
    ...
    {
        return ["Energy System", "Inverter Model", "Inverter Units"
        ];
    }
