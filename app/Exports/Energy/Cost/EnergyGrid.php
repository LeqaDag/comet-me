...
class EnergyGrid implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $grid = DB::table('grid_community_compounds')
            ...
            ->select(
                ...
                'grid_community_compounds.grid_number',
                DB::raw('(grid_community_compounds.grid_cost / grid_community_compounds.grid_number) 
                    as cost_per_unit'),
                'grid_community_compounds.grid_cost as cost'
            )
            ->get();

        $totals = $grid->reduce(function ($carry, $item) {
            $carry['grid_number'] += $item->grid_number;
        ...
        $grid->push([
            'name' => 'Total',
            'model' => '',
            'grid_number' => $totals['grid_number'],
