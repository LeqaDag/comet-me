...
class EnergyPv implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $pvs = DB::table('energy_systems')
            ...
            ->select(
                ...
                'energy_system_pvs.pv_units',
                DB::raw('(energy_system_pvs.cost / energy_system_pvs.pv_units) as cost_per_unit'),
                'energy_system_pvs.cost',
            )
            ->get();

        $totals = $pvs->reduce(function ($carry, $item) {
            $carry['pv_units'] += $item->pv_units;
