...
class EnergyTurbine implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $turbines = DB::table('energy_system_wind_turbines')
            ...
            ->select(
                ...
                'energy_system_wind_turbines.turbine_units',
                DB::raw('(energy_system_wind_turbines.cost / energy_system_wind_turbines.turbine_units) as cost_per_unit'),
                'energy_system_wind_turbines.cost'
            )
            ->get(); 

        $totals = $turbines->reduce(function ($carry, $item) {
            $carry['turbine_units'] += $item->turbine_units;
        ...
        $turbines->push([
            'name' => 'Total',
            'model' => '',
            'turbine_units' => $totals['turbine_units'],
