...
class EnergyElectricityRoom implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $electricityRoom = DB::table('grid_community_compounds')
            ...
            ->select(
                ...
                'grid_community_compounds.electricity_room_number',
                DB::raw('(grid_community_compounds.electricity_room_cost / grid_community_compounds.electricity_room_number) 
                    as cost_per_unit'),
                'grid_community_compounds.electricity_room_cost as cost'
            )
            ->get();

        $totals = $electricityRoom->reduce(function ($carry, $item) {
            $carry['electricity_room_number'] += $item->electricity_room_number;
            $carry['cost_per_unit'] += $item->cost_per_unit;
            $carry['cost'] += $item->cost;

            return $carry;
        }, [
            'electricity_room_number' => 0,
            'cost_per_unit' => 0,
            'cost' => 0,
        ]);

        // Append totals as a new row 
        $electricityRoom->push([
            'name' => 'Total',
            'model' => '',
            'electricity_room_number' => $totals['electricity_room_number'],
