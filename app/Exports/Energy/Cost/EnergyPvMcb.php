...
class EnergyPvMcb implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $pvMcb = DB::table('energy_system_mcb_pvs')
            ...
            ->select(
                ...
                'energy_system_mcb_pvs.mcb_pv_units',
                DB::raw('(energy_system_mcb_pvs.cost / energy_system_mcb_pvs.mcb_pv_units) as cost_per_unit'),
                'energy_system_mcb_pvs.cost'
            )
            ->get();

        $totals = $pvMcb->reduce(function ($carry, $item) {
            $carry['mcb_pv_units'] += $item->mcb_pv_units;
        ...
        $pvMcb->push([
            'name' => 'Total',
            'model' => '',
            'mcb_pv_units' => $totals['mcb_pv_units'],
