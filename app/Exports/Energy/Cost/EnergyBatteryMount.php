...
class EnergyBatteryMount implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        $batteryMounts = DB::table('energy_systems')
            ...
            ->select(
                ...
                'energy_system_battery_mounts.unit',
                DB::raw('(energy_system_battery_mounts.cost / energy_system_battery_mounts.unit) as cost_per_unit'),
                'energy_system_battery_mounts.cost'
            )
            ->get();

        $totals = $batteryMounts->reduce(function ($carry, $item) {
