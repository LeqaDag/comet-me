...
class EnergyBattery implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    public function collection()
    {
        ...
        $batteries = DB::table('energy_systems')
            ...
            ->select(
                ...
                'energy_system_batteries.battery_units',
                DB::raw('(energy_system_batteries.cost / energy_system_batteries.battery_units) as cost_per_unit'),
                'energy_system_batteries.cost'
            )
            ->get();

        $totals = $batteries->reduce(function ($carry, $item) {
            $carry['battery_units'] += $item->battery_units;
        ...
        $batteries->push([
            'name' => 'Total',
            'model' => '',
            'battery_units' => $totals['battery_units'],
