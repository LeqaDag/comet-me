...
class EnergySystemPv implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
    ...
    */
    public function collection()  
    {
        $query = DB::table('energy_systems')
            ...
            ->LeftJoin('energy_system_pvs', 'energy_systems.id', '=', 
                'energy_system_pvs.energy_system_id')
            ->LeftJoin('energy_pvs', 'energy_system_pvs.pv_type_id', '=', 
                'energy_pvs.id')
            ->where('energy_systems.is_archived', 0)
            ->select(
                'energy_systems.name', 
                'energy_pvs.pv_model', 'energy_system_pvs.pv_units',
            );
    ...
    {
        return ["Energy System", "PV Model", "PV Units"
        ];
    }
